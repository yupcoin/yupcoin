...
uint256 hashGenesisBlock("0x31982cd14dee1f81bd8088f36dcd252051e6bcfa8ce468086784df1edaaad9ac");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Yupcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Yupcoin Signed Message:\n";
...
{
    // Yupcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Yupcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Yupcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Yupcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x31982cd14dee1f81bd8088f36dcd252051e6bcfa8ce468086784df1edaaad9ac");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1402328383;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084658258;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("YupcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("YupcoinMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running Yupcoin with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("YupcoinMiner terminated\n");
        throw;
    }
}
